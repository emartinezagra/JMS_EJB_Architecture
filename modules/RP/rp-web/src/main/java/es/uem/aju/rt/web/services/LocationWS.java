...
import es.uem.aju.rp.business.LocationServiceLocal;

@Stateless
@Path("/location")
public class LocationWS extends BaseWS {
	
	@EJB
	LocationServiceLocal locationService;
    ...
    public Response responseFindResource( 
        ...
        if (StringUtils.isNumber(pathParameter)){
        	LocationDTO location = locationService.find(Integer.parseInt(pathParameter));
        	return Response.status(HttpStatus.SC_OK).entity(JSONObjectParser.fromObjectToJson(location)).build();
        }else{
    ...
    public Response responseFindAllRecources() {
		log.debug("Request received by "+this.getClass().getName());		
		LocationListDTO locations = locationService.findAll();
		return Response.status(HttpStatus.SC_OK).entity(JSONObjectParser.fromObjectToJson(locations)).build();					                              
    }
}
